class DataController < ApplicationController
  def index
    ...
    if params[:username].present? && (admin_user.present? && 
                !admin_user.valid_password?(params[:password]) || admin_user.nil?)
        render :text => "Invalid username/password supplied.", :status => :unauthorized 
    else 
